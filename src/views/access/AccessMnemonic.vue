...
import { decryptMnemonic } from "@/service/wallet";
...
function handleAccessMnemonic() {
  const reg = /[^a-zA-Z]/;
  console.log(mnemonic.value);
  const phaseArr = mnemonic.value.split(" ");
  let isPass = true;
  for (let i = 0; i < phaseArr.length; i++) {
    if (reg.test(phaseArr[i])) {
      console.log("reg.test(phaseArr[i]", reg.test(phaseArr[i]));
      isPass = false;
      break;
    }
  }
  if (!mnemonic.value || phaseArr.length != 12 || !isPass) {
    return notification.error({
      title: $t("AccessWallet"),
      content: $t("MnemonicTip"),
      duration: 2000,
    });
  }
  const wallet = decryptMnemonic(mnemonic.value);
  // console.log(wallet);
    ...
    <div class="container p-tb-120 p-tb-sm-80">
      <div class="wf-70 wf-xl-80 wf-sm-100 m-auto">
        ...
        >
          ...
          <div class="ff-tw-400 text-center fs-14 fs-sm-12">
            {{ $t("MnemonicTip") }}
          </div>
          <div
            class="wf-50 wf-lg-60 wf-md-80 wf-sm-100 m-auto m-t-20 m-t-sm-10"
          >
            <div
              class="flex-row-between align-items-start pointer bg-black-p-80 p-15 p-sm-10 radius-10"
            >
              <textarea
                class="hf-100 wf-90 bg-transparent ff-tw-500 fs-16 fs-sm-14 text-white border-none"
                cols="3"
                rows="2"
                :draggable="false"
